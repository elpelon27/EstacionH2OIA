...
def seleccionar_mejor_match(
    pedidos: list[PedidoFinanciero], telefono_emisor: str, monto: float
...
async def procesar_notifica_pago_movil(payload: R4NotificaRequest) -> dict[str, Any]:
    ...
    if payload.CodigoRed != "00":
        logger.warning(
            "R4notifica: CodigoRed != 00 (%s) → rechazado automáticamente", payload.CodigoRed
        )
        return {"abono": False}
...
async def test_flujo_completo():
    ...
    with db.get_db() as conn:
        ...
        conn.execute(
            "DELETE FROM fs_audit_log WHERE registro_id = ? AND tabla = 'fs_pedidos'", (fs_id,)
        )

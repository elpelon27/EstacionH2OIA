...
class SkillRegistry:
    ...
    def discover(self, query: str = "", agent_type: str | None = None, limit: int = 20) -> list[dict[str, str]]:
        results = []
        query_lower = query.lower()

        for skill in self.skills.values():
            if agent_type and skill.scope == SkillScope.AGENT_SPECIFIC:
                if agent_type not in skill.tags and agent_type not in skill.metadata.get('agent_types', []):
                    continue
            ...
            for asset_file in assets_dir.glob('*'):
                if asset_file.is_file():
                    try:
                        ...
                        resources['assets'][asset_file.name] = f"<binary: {asset_file.stat().st_size} bytes>"
    ...
    def get_skills_for_agent(self, agent_type: str) -> list[SkillSpec]:
        relevant = []
        for skill in self.skills.values():
            if (skill.scope == SkillScope.GLOBAL or
                agent_type in skill.tags or
                agent_type in skill.metadata.get('agent_types', [])):
...
def create_skill_registry(skills_dir: str = "/mnt/ssd_trabajo/hermes-agent/skills") -> SkillRegistry:

...
class OdooClient:
    ...
    def get_or_create_partner(self, name: str, vat: str = "", phone: str = "", 
                              street: str = "", city: str = "Maracaibo",
                              country_id: int = 238) -> int:
        """Busca o crea un cliente (partner) por RIF/VAT"""
        # Buscar por VAT si se proporciona
        if vat:
            partners = self.execute_kw('res.partner', 'search_read', 
                [[('vat', '=', vat)]], {'fields': ['id', 'name', 'vat'], 'limit': 1})
            if partners:
        ...
        if phone:
            partners = self.execute_kw('res.partner', 'search_read',
                [[('phone', '=', phone)]], {'fields': ['id', 'name', 'phone'], 'limit': 1})
            if partners:
                logger.info(f"Partner existente por teléfono: {partners[0]['name']} (ID={partners[0]['id']})")
                return partners[0]['id']
    ...
    def get_product_by_name(self, name: str) -> Optional[dict]:
        ...
        products = self.execute_kw('product.template', 'search_read',
            [[('name', '=', name)]], {'fields': ['id', 'name', 'list_price', 'uom_id', 'taxes_id'], 'limit': 1})
        return products[0] if products else None
    
    def get_product_price(self, name: str) -> Optional[float]:
    ...
    def create_sale_order(self, partner_id: int, order_lines: list[dict], 
    ...
    def create_delivery_note(self, partner_id: int, items: list[dict],
                             origin: str = "") -> int:
        """
        ...
        picking_type = self.execute_kw('stock.picking.type', 'search_read',
            [[('code', '=', 'outgoing')]], {'fields': ['id', 'default_location_src_id', 'default_location_dest_id'], 'limit': 1})
        ...
        location_dest_id = pt['default_location_dest_id'][0] if pt['default_location_dest_id'] else 5
        ...
        for item in items:
            ...
            if price_unit is None:
                ...
                if product:
                    ...
                    prod = self.execute_kw('product.template', 'read', [[item['product_id']]], 
                        {'fields': ['list_price']})
    ...
    def confirm_delivery_note(self, picking_id: int) -> bool:
        ...
        try:
            ...
            picking = self.execute_kw('stock.picking', 'read', [[picking_id]], 
                {'fields': ['move_ids_without_package', 'move_line_ids_without_package', 
                           'picking_type_id', 'location_id', 'location_dest_id']})[0]
            
            if picking['move_ids_without_package'] and not picking['move_line_ids_without_package']:
                # Get picking type locations
                pt = self.execute_kw('stock.picking.type', 'read', 
                ...
                for move_id in picking['move_ids_without_package']:
                    move = self.execute_kw('stock.move', 'read', [[move_id]], 
                        {'fields': ['product_id', 'product_uom_qty', 'product_uom', 'product_uom_category_id']})[0]
                    ...
                    move_line_vals = {
                        ...
                        'product_uom_category_id': move['product_uom_category_id'][0] if move['product_uom_category_id'] else 1,
                    }
                    self.execute_kw('stock.move.line', 'create', [move_line_vals])
            
            # 3. Validate the picking
            self.execute_kw('stock.picking', 'button_validate', [[picking_id]], 
    ...
    def convert_delivery_to_invoice(self, picking_id: int, partner_vat: str, 
                                     partner_name: str, partner_street: str) -> Optional[int]:
        """
        ...
        try:
            ...
            picking = self.execute_kw('stock.picking', 'read', [[picking_id]], 
                {'fields': ['move_ids_without_package', 'partner_id', 'origin', 'name']})[0]
            
            # 2. Crear sale.order a partir de los moves
            order_lines = []
            for move_id in picking['move_ids_without_package']:
                move = self.execute_kw('stock.move', 'read', [[move_id]], 
                    {'fields': ['product_id', 'product_uom_qty', 'price_unit', 'name']})[0]
            ...
            partner_data = self.execute_kw('res.partner', 'read', [[picking['partner_id'][0]]], 
                {'fields': ['id', 'name', 'vat', 'street']})[0]
            ...
            order_vals = {
                ...
                'origin': f"ND-{picking.get('origin') or picking['name']}",  # Referencia a nota original
                'note': f"Convertida de nota de entrega {picking['name']}. Inventario ya descontado.",
                'state': 'draft',
            }
            order_id = self.execute_kw('sale.order', 'create', [order_vals])
            ...
            logger.info(f"Nota de entrega {picking['name']} convertida a Sale Order {order_id} + Invoice {invoice_id}")
            return order_id
    ...
    def register_payment(self, invoice_id: int, amount: float, payment_method: str = "pago_movil",
                         reference: str = "", date: str = None) -> Optional[int]:
        """Registra un pago en Odoo para una factura"""
        try:
            # Buscar journal de banco/pagos
            journal = self.execute_kw('account.journal', 'search_read',
                [[('type', '=', 'bank')]], {'fields': ['id'], 'limit': 1})
            if not journal:
                journal = self.execute_kw('account.journal', 'search_read',
                    [[('type', '=', 'cash')]], {'fields': ['id'], 'limit': 1})
            if not journal:
            ...
            payment_vals = {
                ...
                'partner_id': self.execute_kw('account.move', 'read', [[invoice_id]], 
    ...
    def _get_payment_method_id(self, name: str) -> int:
        ...
        methods = self.execute_kw('account.payment.method', 'search_read',
            [[('name', '=', name)]], {'fields': ['id'], 'limit': 1})
        if methods:
            return methods[0]['id']
        # Fallback: primer método disponible
        all_methods = self.execute_kw('account.payment.method', 'search_read',
            [], {'fields': ['id'], 'limit': 1})
    ...
    def get_sales_report(self, date_from: str, date_to: str) -> list:
        ...
        orders = self.execute_kw('sale.order', 'search_read',
            ...
            {'fields': ['name', 'partner_id', 'amount_total', 'date_order', 'state', 'payment_term_id']}
        )
        return orders
    ...
    def get_driver_commissions(self, date_from: str, date_to: str) -> dict:
        ...
        pickings = self.execute_kw('stock.picking', 'search_read',
            [
                ['date_done', '>=', date_from],
                ['date_done', '<=', date_to],
                ['state', '=', 'done'],
    ...
    def get_inventory_levels(self) -> dict:
        ...
        quants = self.execute_kw('stock.quant', 'search_read',

...
from src.integrations.r4.codigos import is_success, get_description
...
# Instancia global
_webhook_config: Optional[R4WebhookConfig] = None
...
class R4ConsultaRequest(BaseModel):
    ...
    TelefonoComercio: str = Field(..., min_length=11, max_length=11, description="Teléfono comercio 11 dígitos")
...
class R4NotificaRequest(BaseModel):
    ...
    BancoEmisor: str = Field(..., min_length=3, max_length=4, description="Código banco 3-4 dígitos")
    Monto: str = Field(..., description="Monto con 2 decimales")
...
async def process_r4consulta(payload: R4ConsultaRequest, config: R4WebhookConfig) -> WebhookProcessResult:
    """
...
async def process_r4notifica(payload: R4NotificaRequest, config: R4WebhookConfig) -> WebhookProcessResult:
    """

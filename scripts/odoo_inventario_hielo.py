...
async def get_hielo_inventory() -> dict:
    ...
    try:
        ...
        products = odoo.execute_kw("product.product", "search_read", [
            ["|", ("name", "ilike", "hielo"), ("name", "ilike", "ice")]
        ], {
            "fields": ["name", "qty_available", "virtual_available", "uom_id", "categ_id", "standard_price", "list_price"]
        })
        
        # Si no encuentra por nombre, buscar por categoría
        if not products:
            # Buscar categoría "Hielo" o "Ice"
            cats = odoo.execute_kw("product.category", "search_read", [
                ["|", ("name", "ilike", "hielo"), ("name", "ilike", "ice")]
            ], {"fields": ["id", "name"], "limit": 5})
            
            for cat in cats:
                cat_products = odoo.execute_kw("product.product", "search_read", [
                    ("categ_id", "=", cat["id"])
                ], {
                    "fields": ["name", "qty_available", "virtual_available", "uom_id", "standard_price", "list_price"]
                })

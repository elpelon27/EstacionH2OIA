...
async def get_nomina_report() -> dict:
    ...
    try:
        ...
        employees = odoo.execute_kw("hr.employee", "search_read", [
            [("active", "=", True)]
        ], {
            "fields": ["name", "job_id", "department_id", "contract_id", "work_email", "mobile_phone"],
            "limit": 100
        })
        
        # Buscar contratos activos
        contracts = odoo.execute_kw("hr.contract", "search_read", [
            [("state", "=", "open")]
        ], {
            "fields": ["name", "employee_id", "wage", "wage_type", "structure_type_id", "date_start", "date_end"],
            "limit": 100
        })
        
        # Buscar estructuras de nómina
        structures = odoo.execute_kw("hr.payroll.structure", "search_read", [], {
            "fields": ["name", "type_id", "rule_ids"],
            "limit": 50
        })

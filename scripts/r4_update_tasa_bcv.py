...
def get_last_tasas() -> dict:
    ...
    try:
        ...
        conn.close()
        return {row["par"]: {"tasa": row["tasa"], "registrado_at": row["registrado_at"]} for row in rows}
    except Exception as e:
...
async def main():
    ...
    saved = False
    if usd is not None and usd != prev_usd:
        saved = save_tasas(usd=usd, eur=eur) or saved
    elif eur is not None and eur != prev_eur:
        saved = save_tasas(usd=usd, eur=eur) or saved

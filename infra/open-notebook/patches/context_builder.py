...
async def build_notebook_context(
    ...
    """
    # Hard cap for the total notebook context. OpenRouter rejects requests
    # above 1,048,576 tokens ("maximum context length"), which the UI then
    # surfaces as "Content too large". 800k leaves headroom for the system
    # prompt, insights, and chat history.
    MAX_NOTEBOOK_CONTEXT_TOKENS = 800_000
        ...
        for source_id, status in context_config.get("sources", {}).items():
            ...
            try:
                ...
                if "insights" in status:
                    ...
                    # Enforce the notebook-wide token cap: skip sources that
                    # would push the total context past the LLM's window.
                    candidate_total = token_count(total_content + str(source_context))
                    if candidate_total > MAX_NOTEBOOK_CONTEXT_TOKENS:
                        logger.warning(
                            f"Skipping source {source_id}: would exceed the "
                            f"{MAX_NOTEBOOK_CONTEXT_TOKENS}-token notebook "
                            f"context cap (candidate: {candidate_total})."
                        )
                        continue
                    context_data["sources"].append(source_context)
                    total_content += str(source_context)
        ...
        for note_id, status in context_config.get("notes", {}).items():
            ...
            try:
                ...
                if "full content" in status:
                    ...
                    candidate_total = token_count(total_content + str(note_context))
                    if candidate_total > MAX_NOTEBOOK_CONTEXT_TOKENS:
                        logger.warning(
                            f"Skipping note {note_id}: would exceed the "
                            f"{MAX_NOTEBOOK_CONTEXT_TOKENS}-token notebook "
                            f"context cap (candidate: {candidate_total})."
                        )
                        continue

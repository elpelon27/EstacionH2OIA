...
class TencentDBMemoryBackend(MemoryBackendInterface):
    ...
    async def _start_service(self) -> bool:
        try:
            ...
            deploy_path = Path("/mnt/ssd_trabajo/hermes-agent/external_repos/memory/TencentDB-Agent-Memory/deploy/global-images/start-all.sh")
            if deploy_path.exists():
                # Configurar .env
                env_path = Path("/mnt/ssd_trabajo/hermes-agent/external_repos/memory/TencentDB-Agent-Memory/deploy/global-images/.env")
                if not env_path.exists():
                    example_path = Path("/mnt/ssd_trabajo/hermes-agent/external_repos/memory/TencentDB-Agent-Memory/deploy/global-images/.env.example")
    ...
    async def add(self, entry: MemoryEntry) -> str:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f"{self.base_url}/api/memory", json=payload, headers=headers) as resp:
    ...
    async def search(
        ...
        async with aiohttp.ClientSession() as session:
            headers = {"Authorization": f"Bearer {self.api_key}"} if self.api_key else {}
            async with session.post(f"{self.base_url}/api/search", json=payload, headers=headers) as resp:
    ...
    async def get(self, entry_id: str) -> Optional[MemoryEntry]:
        ...
        async with aiohttp.ClientSession() as session:
            headers = {"Authorization": f"Bearer {self.api_key}"} if self.api_key else {}
            async with session.get(f"{self.base_url}/api/memory/{entry_id}", headers=headers) as resp:
                if resp.status == 200:
    ...
    async def update(self, entry_id: str, updates: Dict[str, Any]) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            headers = {"Authorization": f"Bearer {self.api_key}"} if self.api_key else {}
            async with session.patch(f"{self.base_url}/api/memory/{entry_id}", json=updates, headers=headers) as resp:
    ...
    async def delete(self, entry_id: str) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            headers = {"Authorization": f"Bearer {self.api_key}"} if self.api_key else {}
            async with session.delete(f"{self.base_url}/api/memory/{entry_id}", headers=headers) as resp:
                return resp.status == 200
...
class Mem0Backend(MemoryBackendInterface):
    ...
    @property
    def supported_types(self) -> List[MemoryType]:
        return [MemoryType.SEMANTIC, MemoryType.EPISODIC, MemoryType.PROCEDURAL, MemoryType.AUTOBIOGRAPHICAL]
    ...
    async def add(self, entry: MemoryEntry) -> str:
        ...
        result = self.client.add(
            messages=messages,
            user_id=metadata.get("user_id", "hermes-agent"),
            metadata=metadata
        )
    ...
    async def initialize(self, config: Dict[str, Any]) -> bool:
        try:
            ...
            agent = self.client.agents.create(
                ...
                system=agent_config.get("persona", "Prometeo - Entidad aislada full-stack para Estación H2O")
            )
...
class UnifiedMemoryManager:
    ...
    def __init__(self):
        ...
        self.routing_rules: Dict[MemoryType, List[MemoryBackend]] = {
            ...
            MemoryType.PROCEDURAL: [MemoryBackend.TENCENTDB, MemoryBackend.MEM0, MemoryBackend.LETTA],
    ...
    def _select_backends(self, memory_type: MemoryType, preferred: Optional[MemoryBackend] = None) -> List[MemoryBackendInterface]:
    ...
    async def add(
        ...
        preferred_backend: Optional[MemoryBackend] = None
    ...
    async def get(self, entry_id: str, backend: Optional[MemoryBackend] = None) -> Optional[MemoryEntry]:

...
from src.banking.r4_endpoints import get_response_meaning, is_bank_ip_allowed
...
async def validate_hmac_signature(
    ...
    commerce: str | None = Header(None, alias="Commerce"),
) -> None:
    """
    Valida firma HMAC-SHA256 del banco.

    Según manual: Authorization = HMAC-SHA256 Hex de
    (string_a_firmar) usando Commerce token como llave.
    ...
    # Por ahora solo log
    logger.info(
        "HMAC validation pending - commerce=%s auth_present=%s",
        commerce[:8] if commerce else None,
        bool(authorization),
    )
    ...
    #     commerce_token_bytes, string_to_sign.encode(), hashlib.sha256
    # ).hexdigest()
...
async def webhook_r4_notifica(
    ...
    if not pedidos:
        logger.info(
            "No hay pedido pendiente para emisor=%s monto=%.2f", telefono_emisor, monto
        )
        return R4NotificaResponse(abono=False)
    ...
    if not pedido:
        logger.warning(
            "No se pudo seleccionar match único entre %d candidatos", len(pedidos)
        )
        return R4NotificaResponse(abono=False)
    ...
    try:
        ...
        if resultado.get("verificado"):
            logger.info(
                "Pago móvil VERIFICADO por banco R4: fs_pedido=%d ref=%s",
                fs_pedido_id,
                payload.Referencia,
            )
...
async def webhook_mb_consulta(
    ...
    logger.info(
        "MBconsulta recibido: comercio=%s emisor=%s monto=%s",
        payload.IdComercio,
        payload.TelefonoEmisor,
        payload.Monto,
    )

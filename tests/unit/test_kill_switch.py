...
def test_bridge_health_kill_switch(reset_prometheus):
    ...
    from api.bridge import _is_kill_switch_active, KILL_SWITCH_FILE
    
    # Test with no kill switch
    if os.path.exists(KILL_SWITCH_FILE):
        os.remove(KILL_SWITCH_FILE)
    
    assert _is_kill_switch_active() == False
    
    # Test with kill switch active
    with open(KILL_SWITCH_FILE, 'w') as f:
        f.write("test")
    
    assert _is_kill_switch_active() == True

...
def clean_dispatch_queue():
    ...
    conn.execute('DELETE FROM deliveries WHERE client_id IN (SELECT id FROM clients WHERE phone LIKE "+58412%")')
...
async def test_full_e2e_flow_bridge_to_consumer():
    ...
    conn.row_factory = sqlite3.Row
    client = conn.execute(
        'SELECT * FROM clients WHERE phone = ?', (from_phone,)
    ).fetchone()
    conn.close()
...
async def test_multiple_orders_batch():
    ...
    for i, (ph_suffix, bot, hielo, total, metodo) in enumerate(orders_data):

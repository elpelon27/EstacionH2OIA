...
def test_db():
    ...
    # Insert test zones
    conn.execute("INSERT INTO zones (id, name, center_lat, center_lng, radius_km) VALUES (1, 'Norte', 10.6700, -71.6300, 8.0)")
    conn.execute("INSERT INTO zones (id, name, center_lat, center_lng, radius_km) VALUES (2, 'Centro', 10.6447, -71.6101, 5.0)")
    conn.execute("INSERT INTO zones (id, name, center_lat, center_lng, radius_km) VALUES (3, 'Sur-Este', 10.6100, -71.5800, 7.0)")
    
    # Insert test vehicles (no chat_id initially)
    conn.execute("INSERT INTO vehicles (id, name, operator_name, telegram_chat_id, max_full_bottles, max_empty_bottles, active) VALUES (1, 'Triciclo 1', 'YORDANIS', NULL, 30, 70, 1)")
    conn.execute("INSERT INTO vehicles (id, name, operator_name, telegram_chat_id, max_full_bottles, max_empty_bottles, active) VALUES (2, 'Triciclo 2', 'EVERT', NULL, 30, 70, 1)")
...
class TestSprint43E2E:
    ...
    async def test_consumer_creates_delivery_and_notifies(self):
        ...
        conv_db.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)
        """, ("Test Consumer Client", "+584129999999", "3 botellones de agua", 
              3.0, 108.0, "efectivo", 10.65, -71.62, "Test Address", now))
        conv_db.commit()
        conv_db.close()
    ...
    async def test_driver_actions_llegue_entregado(self):
        ...
        conn = get_dispatch_db()
        delivery = conn.execute("SELECT status, actual_arrival FROM deliveries WHERE id = ?", (delivery_id,)).fetchone()
        conn.close()
        ...
        conn = get_dispatch_db()
        delivery = conn.execute("SELECT status, actual_departure FROM deliveries WHERE id = ?", (delivery_id,)).fetchone()
        conn.close()
